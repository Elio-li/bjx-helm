        ...
        stage('Rollback Check') {
            ...
            steps {
                script {
                    ...
                    def versions = sh(
                        script: """
                            echo '${versionsJson}' | jq -r '.[] | select(.status=="deployed" or .status=="superseded") | .app_version' | grep -v null | grep -v '^\
...
def rollbackDeployment(String release, String ns) {
    ...
    try {
        ...
        def prevRev = sh(
            script: """
                echo '${historyJson}' | jq -r '[.[] | select(.status=="deployed" or .status=="superseded")] | sort_by(.revision) | .[-2].revision // empty'
            """,
            returnStdout: true
        ).trim()
